...
static unsigned int gpioButton = 24;    // pin 13 (GPIO24)
...
static irq_handler_t erpi_gpio_irq_handler(unsigned int irq,
    ...
    gpio_set_value(gpioLED, true);     // set LED accordingly
    msleep(2000);                      // Sleep in IRQ handler - Not good at all!!!
    gpio_set_value(gpioLED, false);     // set LED accordingly
